...
public class WISERBizManage implements IWISERBizManage {
	...
	@Override
    public void attach(WISERBizModel model) {
		synchronized (bizs) {
	...
	@Override
    public void detach(WISERBizModel model) {
		synchronized (bizs) {
			if (model == null) return;
	...
	@Override
    public <B extends IWISERBiz> boolean isExist(Class<B> bizClazz) {
	...
	@Override
    public <B extends IWISERBiz> B biz(Class<B> bizClazz) {
